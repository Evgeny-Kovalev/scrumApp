...
export class IterationService {
  ...
  getIterationById(iterationId: string): Observable<Iteration> {
    ...
    .pipe(
      // take(1),
      map((iteration: any) => {
        const {_id, projectId, startTime, finishTime} = iteration
    ...
    .pipe(
      map((iterations: any) => {
        return iterations.map((iteration: any) => {
          return new Iteration(iteration._id, iteration.projectId, iteration.startTime, iteration.finishTime)
        })
      })
    )
  }
  ...
  createIteration(projectId: string, iteration: {startTime: Date, finishTime: Date}): Observable<Iteration>{
    ...
    .pipe(
      map((res: any) => {
        const {_id, projectId, startTime, finishTime} = res.iteration
        return new Iteration(_id, projectId, new Date(startTime), new Date(finishTime))
      })
    )
  }
